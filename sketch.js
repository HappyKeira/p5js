...
function draw() {
 ...
 if (topCountries.includes(country)) {
      ...
      let baseX = map(row.getNum('WaterQuality'), 0, 100, plotX, plotX + plotWidth);
      let baseY = map(row.getNum('AirQuality'), 0, 100, plotY + plotHeight, plotY);
      
      // Add trembling effect
      let trembleAmount = 2; 
      let trembleX = random(-trembleAmount, trembleAmount);
      let trembleY = random(-trembleAmount, trembleAmount);
      
      let x = baseX + trembleX;
      let y = baseY + trembleY;
 }
